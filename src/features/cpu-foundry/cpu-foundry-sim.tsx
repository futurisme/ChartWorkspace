...
function calculateLaunchRevenue(score: number, teams: Snapshot['teams'], marketShare: number, reputation: number) {
  ...
  return score * 0.9 * fabMultiplier * marketingMultiplier * marketMultiplier * reputationMultiplier;
}

export function CpuFoundrySim() {
  const [state, setState] = useState<Snapshot>(STARTING_STATE);
  const [secondsElapsed, setSecondsElapsed] = useState(0);
  const [launchMessage, setLaunchMessage] = useState('Rancang CPU flagship pertama untuk membangun momentum perusahaan.');
  ...
  const launchCpu = () => {
    const launchRevenue = calculateLaunchRevenue(cpuScore, state.teams, state.marketShare, state.reputation);
    const reputationGain = Math.max(1.5, cpuScore / 240 + state.teams.marketing.count * 0.8);
    const marketShareGain = Math.min(4.2, cpuScore / 500 + state.teams.fabrication.count * 0.15);
    ...
    setLaunchMessage(`${flagshipName} meluncur pada ${state.year}. Pendapatan +$${formatNumber(launchRevenue, 0)}M dan reputasi meningkat.`);
  };
  ...
  const strategySignals = [
    {
      label: 'Riset / detik',
      value: `${formatNumber(state.researchPerSecond, 1)} RP/s`,
      tone: 'cyan',
    },
    {
      label: 'Cashflow / detik',
      value: `$${formatNumber(state.revenuePerSecond, 1)}M/s`,
      tone: 'amber',
    },
    {
      label: 'Pangsa pasar',
      value: `${formatNumber(state.marketShare, 1)}%`,
      tone: 'purple',
    },
    {
      label: 'Reputasi',
      value: `${formatNumber(state.reputation, 1)} / 100`,
      tone: 'emerald',
    },
  ] as const;
  ...
  return (
    <main className={styles.shell}>
      <section className={styles.hero}>
        <div>
          <p className={styles.eyebrow}>/game · isolated CPU company simulator</p>
          <h1>{state.companyName}</h1>
          <p className={styles.subtitle}>
            Mulai dari startup CPU pada tahun 2000, kumpulkan research point per detik, lalu dorong spesifikasi prosesor baru
            sampai perusahaanmu menjadi raksasa teknologi.
          </p>
        </div>
      ...
      <section className={styles.signalGrid}>
        {strategySignals.map((signal) => (
          <article key={signal.label} className={`${styles.signalCard} ${styles[`tone${signal.tone[0].toUpperCase()}${signal.tone.slice(1)}`]}`}>
      ...
      <section className={styles.columns}>
        <section className={styles.panel}>
          <div className={styles.panelHeader}>
            <div>
              <p className={styles.sectionLabel}>Executive board</p>
              <h2>Perusahaan & momentum</h2>
            </div>
            <button type="button" className={styles.launchButton} onClick={launchCpu}>
              Launch CPU
            </button>
          </div>
          ...
          <div className={styles.resourceGrid}>
            <div className={styles.resourceCard}>
              <span>Dana</span>
              <strong>$ {formatNumber(state.funds, 1)}M</strong>
              <small>Dipakai untuk ekspansi tim dan lini produksi.</small>
            </div>
            <div className={styles.resourceCard}>
              <span>Research</span>
              <strong>{formatNumber(state.research, 1)} RP</strong>
              <small>Digunakan untuk meningkatkan spesifikasi CPU.</small>
            </div>
            <div className={styles.resourceCard}>
              <span>Rilis CPU</span>
              <strong>{formatNumber(state.releaseCount)}</strong>
              <small>Setiap peluncuran menaikkan market share dan brand.</small>
            </div>
            <div className={styles.resourceCard}>
              <span>Best score</span>
              <strong>{formatNumber(Math.max(cpuScore, state.bestCpuScore), 0)}</strong>
              <small>Benchmark internal terbaik dari seluruh generasi.</small>
            </div>
          </div>

          <div className={styles.messageBox}>
            <p className={styles.sectionLabel}>Board memo</p>
            <p>{launchMessage}</p>
          </div>
        </section>
        ...
        <section className={styles.panel}>
          <div className={styles.panelHeader}>
            <div>
              <p className={styles.sectionLabel}>R&D complex</p>
              <h2>Peningkatan spesifikasi CPU</h2>
            </div>
            <span className={styles.headerMeta}>Naikkan spek memakai research point</span>
          </div>
              ...
              return (
                <article key={key} className={styles.itemCard}>
                  ...
                  <div className={styles.itemFooter}>
                    <span>Biaya riset: {formatNumber(cost)} RP</span>
                    <button type="button" onClick={() => improveUpgrade(key)} disabled={!afford}>
                      {afford ? 'Improve' : 'Research kurang'}
                    </button>
                  </div>
                </article>
              );
        ...
        <section className={styles.panel}>
          <div className={styles.panelHeader}>
            <div>
              <p className={styles.sectionLabel}>Expansion programs</p>
              <h2>Tim & fasilitas</h2>
            </div>
            <span className={styles.headerMeta}>Beli dengan dana perusahaan</span>
          </div>
            ...
            {(Object.entries(state.teams) as [TeamKey, TeamState][]).map(([key, team]) => {
              ...
              return (
                <article key={key} className={styles.itemCard}>
                  ...
                  <div className={styles.itemFooter}>
                    <span>Biaya ekspansi: ${formatNumber(cost)}M</span>
                    <button type="button" onClick={() => hireTeam(key)} disabled={!afford}>
